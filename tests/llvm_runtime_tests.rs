...
fn llvm_backend_builds_and_runs_zero_division_error_program_on_windows() {
    ...
    let exe_path = dir.join("llvm_zero_division_demo.exe");
    ...
    build_executable_llvm(&source_path, &exe_path, Some("x86_64-pc-windows-gnu"))
        .expect("llvm zero division program should build");

    let output = Command::new(&exe_path)
        .output()
        .expect("failed to run llvm-built zero division executable");
...
#[test]
fn llvm_backend_builds_and_runs_serial_flush_program_on_windows() {
    let dir = temp_dir();
    let source_path = dir.join("llvm_serial_flush_demo.rn");
    let exe_path = dir.join("llvm_serial_flush_demo.exe");
    ...
    build_executable_llvm(&source_path, &exe_path, Some("x86_64-pc-windows-gnu"))
        .expect("llvm serial flush program should build");

    let output = Command::new(&exe_path)
        .output()
        .expect("failed to run llvm-built serial flush executable");
...
#[test]
fn llvm_backend_builds_and_runs_serial_byte_helpers_program_on_windows() {
    let dir = temp_dir();
    let source_path = dir.join("llvm_serial_byte_helpers_demo.rn");
    let exe_path = dir.join("llvm_serial_byte_helpers_demo.exe");
...
def main() -> i32:\n    println(peek_byte())\n    println(write_byte(65))\n    return 0\n",
    ...
    build_executable_llvm(&source_path, &exe_path, Some("x86_64-pc-windows-gnu"))
        .expect("llvm serial byte helper program should build");

    let output = Command::new(&exe_path)
        .output()
        .expect("failed to run llvm-built serial byte helper executable");
...
#[test]
fn llvm_backend_builds_and_runs_serial_available_and_read_byte_program_on_windows() {
    let dir = temp_dir();
    let source_path = dir.join("llvm_serial_available_read_byte_demo.rn");
    let exe_path = dir.join("llvm_serial_available_read_byte_demo.exe");
...
def main() -> i32:\n    println(available())\n    println(read_byte())\n    return 0\n",
    ...
    build_executable_llvm(&source_path, &exe_path, Some("x86_64-pc-windows-gnu"))
        .expect("llvm serial available/read_byte program should build");

    let output = Command::new(&exe_path)
        .output()
        .expect("failed to run llvm-built serial available/read_byte executable");
...
#[test]
fn llvm_backend_builds_and_runs_fs_extended_program_on_windows() {
    let dir = temp_dir();
    let source_path = dir.join("llvm_fs_extended.rn");
    let exe_path = dir.join("llvm_fs_extended.exe");
...
"#,
    ...
    build_executable_llvm(&source_path, &exe_path, Some("x86_64-pc-windows-gnu"))
        .expect("llvm extended fs program should build");

    let output = Command::new(&exe_path)
        .output()
        .expect("failed to run llvm-built extended fs executable");
...
#[test]
fn llvm_backend_builds_and_runs_arduino_uart_tone_and_shift_helpers_program_on_windows() {
    let dir = temp_dir();
    let source_path = dir.join("llvm_arduino_uart_tone_shift.rn");
    let exe_path = dir.join("llvm_arduino_uart_tone_shift.exe");
    ...
    build_executable_llvm(&source_path, &exe_path, Some("x86_64-pc-windows-gnu"))
        .expect("llvm arduino uart/tone/shift helper program should build");
...
#[test]
fn llvm_backend_builds_and_runs_arduino_uart_and_ultrasonic_helpers_program_on_windows() {
    let dir = temp_dir();
    let source_path = dir.join("llvm_arduino_uart_ultrasonic.rn");
    let exe_path = dir.join("llvm_arduino_uart_ultrasonic.exe");
    ...
    build_executable_llvm(&source_path, &exe_path, Some("x86_64-pc-windows-gnu"))
        .expect("llvm arduino uart/ultrasonic helper program should build");
...
#[test]
fn llvm_backend_builds_and_runs_arduino_spi_and_i2c_helpers_program_on_windows() {
    let dir = temp_dir();
    let source_path = dir.join("llvm_arduino_spi_i2c.rn");
    let exe_path = dir.join("llvm_arduino_spi_i2c.exe");
    ...
    build_executable_llvm(&source_path, &exe_path, Some("x86_64-pc-windows-gnu"))
        .expect("llvm arduino spi/i2c helper program should build");
...
#[test]
fn llvm_backend_builds_and_runs_arduino_pulse_input_helpers_program_on_windows() {
    let dir = temp_dir();
    let source_path = dir.join("llvm_arduino_pulse_input.rn");
    let exe_path = dir.join("llvm_arduino_pulse_input.exe");
    ...
    build_executable_llvm(&source_path, &exe_path, Some("x86_64-pc-windows-gnu"))
        .expect("llvm arduino pulse input helper program should build");
...
#[test]
fn llvm_backend_builds_and_runs_builtin_pwm_and_adc_program_on_windows() {
    let dir = temp_dir();
    let source_path = dir.join("llvm_builtin_pwm_adc_demo.rn");
    let exe_path = dir.join("llvm_builtin_pwm_adc_demo.exe");
...
def main() -> i32:\n    let pwm = pwm_pin(9)\n    let sensor = adc_pin(9)\n    pwm.output()\n    pwm.write(64)\n    println(sensor.read())\n    println(sensor.read_percent())\n    println(sensor.read_voltage_mv(5000))\n    println(pwm.max_duty())\n    println(max())\n    return 0\n",
    ...
    build_executable_llvm(&source_path, &exe_path, Some("x86_64-pc-windows-gnu"))
        .expect("llvm builtin pwm/adc program should build");

    let output = Command::new(&exe_path)
        .output()
        .expect("failed to run llvm builtin pwm/adc executable");
...
#[test]
fn llvm_backend_builds_and_runs_namespaced_builtin_pwm_and_adc_program_on_windows() {
    let dir = temp_dir();
    let source_path = dir.join("llvm_namespaced_builtin_pwm_adc_demo.rn");
    let exe_path = dir.join("llvm_namespaced_builtin_pwm_adc_demo.exe");
...
def main() -> i32:\n    let out = pwm.pin(9)\n    let sensor = adc.pin(9)\n    out.output()\n    out.write(64)\n    println(sensor.read())\n    println(sensor.read_percent())\n    println(out.max_duty())\n    println(adc.max())\n    return 0\n",
    ...
    build_executable_llvm(&source_path, &exe_path, Some("x86_64-pc-windows-gnu"))
        .expect("llvm namespaced builtin pwm/adc program should build");

    let output = Command::new(&exe_path)
        .output()
        .expect("failed to run llvm namespaced builtin pwm/adc executable");
...
#[test]
fn llvm_backend_builds_and_runs_namespaced_local_modules_with_overlapping_exports_on_windows() {
    let dir = temp_dir();
    let source_path = dir.join("main.rn");
    let exe_path = dir.join("llvm_namespaced_local_modules.exe");
    ...
    build_executable_llvm(&source_path, &exe_path, Some("x86_64-pc-windows-gnu"))
        .expect("llvm namespaced local module program should build");

    let output = Command::new(&exe_path)
        .output()
        .expect("failed to run llvm namespaced local module executable");
