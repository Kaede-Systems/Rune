...
void CWriter::generateHeader(Module &M) {
  ...
  if (!M.alias_empty()) {
    ...
    for (Module::alias_iterator I = M.alias_begin(), E = M.alias_end(); I != E;
      ...
      if (const auto *AliaseeFn = dyn_cast<Function>(I->getAliaseeObject())) {
        Alignment = AliaseeFn->getAlign().valueOrOne().value();
      } else if (const auto *AliaseeVar =
                     dyn_cast<GlobalVariable>(I->getAliaseeObject())) {
        Alignment = AliaseeVar->getAlign().valueOrOne().value();
      }
      bool IsOveraligned =
          Alignment && Alignment > TD->getABITypeAlign(ElTy).value();
      if (IsOveraligned) {
